        ...
        case 'book':
            if(isset($_POST["data"])){
                ...
                try {
                    ...
                    $mail->Host       = $parser->getMailerConfig()["server"];   // Set the SMTP server to send through
                    $mail->SMTPAuth   = true;                                   // Enable SMTP authentication
                    $mail->Username   = $parser->getMailerConfig()["username"]; // SMTP username
                    $mail->Password   = $parser->getMailerConfig()["password"]; // SMTP password
                    $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;         // Enable TLS encryption; `PHPMailer::ENCRYPTION_SMTPS` encouraged
                    $mail->Port       = 587;                                    // TCP port to connect to, use 465 for `PHPMailer::ENCRYPTION_SMTPS` above
                    $mail->CharSet    = 'UTF-8';                                // Uses UTF-8 encoding format for the mail

                    //Recipients
                    $mail->setFrom($parser->getMailerConfig()["from"]["address"], $parser->getMailerConfig()["from"]["name"]);
                    $mail->addAddress($appointment->getParentEmail(), "M. ou Mme ".$appointment->getParentName());     // Add a recipient
                    $mail->addReplyTo($parser->getMailerConfig()["from"]["address"], $parser->getMailerConfig()["from"]["name"]);
                    //$mail->addCC('cc@example.com');
                    foreach ($parser->getMailerConfig()["bcc"] as $key => $value) {
